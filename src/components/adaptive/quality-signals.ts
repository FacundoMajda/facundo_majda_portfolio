...
import {
  ...
  type FrameHealthMonitor,
  type FrameHealthSample,
} from "./frame-health";
...
export function createQualitySignals(
  ...
  void Promise.resolve()
    ...
    .catch((error: unknown) => {
      if (!disposed) {
        logger.info("[quality] GPU detection unavailable.", {
  ...
  if (!browserNavigator?.getBattery) {
    ...
    try {
      ...
      logger.info("[quality] Battery status unavailable.", {
        reason: "rejected",
        error: errorMessage(error),
      });
    }
  ...
  return {
    recordFrame(sample) {
      ...
      logger.info("[quality] Frame health below target.", {
        ...
        observedFps:
          status.observedFps === undefined ? undefined : round(status.observedFps),

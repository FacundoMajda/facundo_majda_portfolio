  ...
  const record = (sample: FrameHealthSample): FrameHealthStatus => {
    if (downgrade) {
      return status(
        true,
        refreshFps,
        activeTargetFps ?? DEFAULT_REFRESH_FPS,
    ...
    while (
      ...
      activeDurationMs - activeFrames[0]!.durationMs >= windowMs
    ) {
    ...
    if (activeDurationMs >= windowMs && observedFps < target * ratio)
      downgrade = true;
